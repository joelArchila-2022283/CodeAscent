...
export class TSDashboardComponent implements OnInit {
  ...
  misiones = signal<IMisionTS[]>([]);
  retoSeleccionado = signal<IReto | null>(null);
  jugadorCargando = signal(true);
  player = { name: '', level: 1, currentXp: 0, nextLevelXp: 100, energyWatts: 86 };
  cartoonMascotState = signal<MascotState>('idle');
  mascotDialogue = signal('El sector TS está listo para trabajar.');
  ...
  private cargarJugador(): void {
    this.dashboardService.obtenerDatosDashboard().subscribe({
      next: data => {
        ...
        this.misiones.set(data?.misiones ?? []);
    ...
    this.tsDataService.obtenerContexto().subscribe({
      ...
      error: err => console.error('Error al cargar el progreso del HUD:', err)
    });
  }
  ...
  seleccionarReto(reto: IReto): void {
    ...
    this.mascotDialogue.set('Misión seleccionada. El terminal está esperando tu solución.');
  ...
  navigateTo(section: TSSection): void {
    ...
    if (section !== 'terminal') this.retoSeleccionado.set(null);
